...
Soft counterpart to ``constrained/plane_with_obstacle.py``.  The
height-pinning residual ``tcp_z - z0`` is squared into a scalar cost
that RRT* integrates along every motion, and a solid red sphere sits
directly in the gripper's preferred sweep.  Collision avoidance is
still a hard constraint (handled by VAMP); only the height preference
is soft — so the planner bends the arm around the sphere while
keeping the gripper as close to the plane as the optimiser can
manage.
...
from fetch_planning.planning import Cost, create_planner
