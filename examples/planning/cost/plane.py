"""Plane cost: the gripper is encouraged to stay at a fixed height.

Soft counterpart to ``constrained/plane.py``.  The one-line residual
``tcp_z - z0`` becomes ``(tcp_z - z0)^2`` — a single-term quadratic
penalty that RRT* trapezoidally integrates along every motion.  The
optimised path keeps the gripper very near the plane without being
locked to it; short off-plane excursions are allowed whenever they're
