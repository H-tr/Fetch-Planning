...
def create_ik_solver(
    ...
    *,
    ...
    Input:
        chain_name: Name of the chain (e.g. "arm", "arm_with_torso", "whole_body").
        config: IK configuration — ``IKConfig`` for trac_ik/ikfast,
            ``PinkIKConfig`` for pink (uses defaults if None).
        side: Optional side suffix for compound names (unused for Fetch, which
            is a single-arm robot; kept for API compatibility).
        urdf_path: Override the default URDF file path.
        backend: ``"trac_ik"`` (default), ``"pink"``, or ``"ikfast"``.
    ...
    Output:
        An IKSolverBase instance (TracIKSolver, PinkIKSolver, or IKFastSolver).

    Examples:
        create_ik_solver("arm")
        create_ik_solver("arm_with_torso", backend="ikfast")
        create_ik_solver("whole_body", backend="pink",
    ...
    if backend == "ikfast":
        # IKFast is analytic and only supports the 8-DOF arm_with_torso chain
        # (2 free parameters: torso_lift + upperarm_roll). Other chains are
        # not supported by this backend.
        if chain_name not in {"arm_with_torso", "arm"}:
            raise ValueError(
                f"ikfast backend only supports 'arm' or 'arm_with_torso' chains, "
                f"not '{chain_name}'. Use 'trac_ik' or 'pink' for other chains."
            )
        from fetch_planning.kinematics.ikfast_solver import IKFastSolver

        ik_config = config if isinstance(config, IKConfig) else None
        return IKFastSolver(chain_config, ik_config)

    raise ValueError(
        f"Unknown backend '{backend}'. Choose 'trac_ik', 'pink', or 'ikfast'."
    )
