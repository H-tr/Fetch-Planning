...
def _cache_root() -> Path:
    ...
    return (base / "fetch_planning" / "constraints").resolve()
...
class SymbolicContext:
    ...
    def __init__(
        ...
        full_names = list(fetch_robot_config.joint_names)
        if subgroup in ("fetch", "fetch_whole_body"):
        ...
        # Numeric + symbolic pinocchio models — loaded from the whole-body
        # URDF so the 3 base joints are real kinematic DOFs of the tree.
        self.pin_model = pin.buildModelFromUrdf(WHOLE_BODY_URDF_PATH)
    ...
    def _build_pinocchio_q(self, q_active: ca.SX) -> ca.SX:
        ...
        The whole-body URDF has a straight 11-joint chain, so the
        pinocchio q-vector is simply the full 11-DOF body in URDF order:
        ``[base_x, base_y, base_theta, torso_lift, shoulder_pan, …,
        wrist_roll]``.
        """
...
class Constraint:
    ...
    Constructing this class triggers (on cold cache):
        ...
        4. caching under ``~/.cache/fetch_planning/constraints/<sha>/``
