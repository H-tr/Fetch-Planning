...
def _cache_root() -> Path:
    ...
    Honours ``FETCH_PLANNING_COST_CACHE_DIR`` if set.  Otherwise falls
    back to ``~/.cache/fetch_planning/costs``.
    """
    override = os.environ.get("FETCH_PLANNING_COST_CACHE_DIR")
    if override:
        return Path(override).expanduser().resolve()
    xdg = os.environ.get("XDG_CACHE_HOME")
    base = Path(xdg).expanduser() if xdg else Path.home() / ".cache"
    return (base / "fetch_planning" / "costs").resolve()
...
class Cost:
    ...
    vector.  Construction triggers (on cold cache):
        ...
        4. caching under ``~/.cache/fetch_planning/costs/<sha>/``
    ...
    def __post_init__(self) -> None:
        ...
        if not so_path.exists():
            sys.stderr.write(f"[fetch_planning] compiling cost {sha[:8]}... ")
            sys.stderr.flush()
            t0 = time.perf_counter()
            with _cwd(cache_dir):
                f.generate("cost.c")
            compiler = os.environ.get("FETCH_PLANNING_COST_CC", "c++")
            subprocess.run(
                [
                    compiler,
                    "-O3",
