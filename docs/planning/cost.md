...
The example demos in `examples/planning/cost/` use `EE_LINK =
"gripper_link"` for orientation terms and the midpoint of
`l_gripper_finger_link` / `r_gripper_finger_link` for the TCP
translation — see `examples/planning/cost/_shared.py` for the
helpers.
